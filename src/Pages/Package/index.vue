  ...
  <DxDataGrid
    ...
    <template #operationCell="cell">
      <div class="asc__listPage-operations">
        ...
        <router-link :to="{name: 'PackageUpdate', params: {slug: cell.data.value}}" v-b-tooltip.hover :title="$t('list.setttings')"><i class="fas fa-cog"></i></router-link>
      </div>
    </template>
    ...
    <template #simonlyCell="cell">
      <span :class="cell.data.value === '1' ? 'btn-info text-center d-block mx-auto mb-0 text-light rounded-sm py-1' : 'btn-warning text-center d-block mx-auto mb-0 rounded-sm py-1'">
        <i :class="cell.data.value === '1' ? 'fas fa-sim-card' : 'fas fa-mobile'"></i> <span>{{$t(cell.data.value === '1' ? 'Package.yesSimonly' : 'Package.withPhone')}}</span>
      </span>
    </template>
...
export default {
  data () {
    return {
      ...
      selectedRow: '',
      rows: [
        {field: 'simonly', caption: 'simonly', format: '', dataType: '', alignment: '', cellTemplate: 'simonlyCell'},
        {field: 'tariffTitle', caption: 'tariffTitle', format: '', dataType: '', alignment: '', cellTemplate: ''},
        {field: 'title', caption: 'title', format: '', dataType: '', alignment: '', cellTemplate: ''},
        {field: 'price', caption: 'price', format: 'currency', dataType: '', alignment: 'right', cellTemplate: 'calculateCell'},
        {field: 'commission', caption: 'commission', format: 'currency', dataType: '', alignment: 'right', cellTemplate: 'calculateCell'},
