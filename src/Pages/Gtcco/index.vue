<template>
  <DxDataGrid
    ...
    <DxColumn v-if="tableRows[0].visible" :data-field="tableRows[0].field" :caption="$t( 'Gtcco.' + tableRows[0].field)" :format="tableRows[0].format" :data-type="tableRows[0].dataType" :alignment="tableRows[0].alignment" :cell-template="tableRows[0].cellTemplate" />
    <DxColumn v-if="tableRows[1].visible" :data-field="tableRows[1].field" :caption="$t( 'Gtcco.' + tableRows[1].field)" :format="tableRows[1].format" :data-type="tableRows[1].dataType" :alignment="tableRows[1].alignment" :cell-template="tableRows[1].cellTemplate" />
    <DxColumn v-if="tableRows[2].visible" :data-field="tableRows[2].field" :caption="$t( 'Gtcco.' + tableRows[2].field)" :format="tableRows[2].format" :data-type="tableRows[2].dataType" :alignment="tableRows[2].alignment" :cell-template="tableRows[2].cellTemplate" />
    <DxColumn v-if="tableRows[3].visible" :data-field="tableRows[3].field" :caption="$t( 'Gtcco.' + tableRows[3].field)" :format="tableRows[3].format" :data-type="tableRows[3].dataType" :alignment="tableRows[3].alignment" :cell-template="tableRows[3].cellTemplate" />
...
export default {
  ...
  watch: {
    '$route' (to) {
      let url = to.name === 'gradingTool' ? 'gtcco?gradingTool=' + to.params.gradingTool : to.name === 'courseOutcome' ? 'gtcco?courseOutcome=' + to.params.courseOutcome : 'gtcco'
      this.getData(url)
    }
  },
  mounted () {
    let url = this.$route.params.name === 'gradingTool' ? 'gtcco?gradingTool=' + this.$route.params.gradingTool : this.$route.params.name === 'courseOutcome' ? 'gtcco?courseOutcome=' + this.$route.params.courseOutcome : 'gtcco'
    this.getData(url)
    this.setRows()
  },
  methods: {
