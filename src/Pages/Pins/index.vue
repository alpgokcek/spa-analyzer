  ...
  <DxDataGrid
    ...
    <template #operationCell="cell">
      <div class="asc__listPage-operations">
        <router-link :to="{name: 'PinCodeUpdate', params: {slug: cell.data.value}}" v-b-tooltip.hover :title="$t('list.pincodes')"><i class="fas fa-fingerprint"></i></router-link>
        <router-link :to="{name: 'PinsUpdate', params: {slug: cell.data.value}}" v-b-tooltip.hover :title="$t('list.setttings')"><i class="fas fa-cog"></i></router-link>
      </div>
    </template>
  ...
  data () {
    return {
      ...
      rows: [
        ...
        {field: 'discount', caption: 'discount', format: '', dataType: '', alignment: 'right', cellTemplate: 'discountCell'},
        {field: 'token', caption: 'operations', format: '', dataType: '', alignment: '', cellTemplate: 'operationCell'}
      ],
      setTableActions: [
        { id: 1, text: this.$t('list.setttings'), icon: 'edit', route: 'PinsUpdate' }
      ]
    }
  },
  computed: {
