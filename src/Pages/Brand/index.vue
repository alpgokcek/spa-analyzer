<template>
  <DxDataGrid
    ...
    <DxColumn v-if="tableRows[5].visible" :data-field="tableRows[5].field" :caption="$t( routeName + '.' + tableRows[5].field)" :format="tableRows[5].format" :data-type="tableRows[5].dataType" :alignment="tableRows[5].alignment" :cell-template="tableRows[5].cellTemplate" />
...
export default {
  data () {
    return {
      ...
      rows: [
        ...
        {visible: true, field: 'summary', format: '', dataType: '', alignment: 'left', cellTemplate: ''},
        {visible: true, field: 'logo', format: '', dataType: '', alignment: '', cellTemplate: 'logoCell'},
        {visible: true, field: 'type', format: '', dataType: '', alignment: '', cellTemplate: ''},
  ...
  watch: {
    '$route' (to) {
      this.getData('settings/brand?type=' + to.params.type)
    }
  },
  mounted () {
    this.getData('settings/brand?type=' + this.$route.params.type)
    this.setRows()
  },
  methods: {
    setRows () {
