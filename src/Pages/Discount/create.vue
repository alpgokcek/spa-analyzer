    ...
    <b-row class="asc__insertPage-body">
      <b-col cols="12" sm="12" md="12" lg="4" xl="4" class="mb-3">
        <b-card :title="$t('Phone.general')" class="shadow">
          ...
          <b-form-group :label="$t('Package.isSimOnly')">
            <b-form-checkbox v-model="simonly" name="check-button" switch>
              {{$t('Package.isSimOnly')}} -- {{form.simonly}} - {{simonly}}
            </b-form-checkbox>
          </b-form-group>
          ...
          <b-form-group :label="$t('Package.commission')" :class="$v.form.commission.$error && !$v.form.commission.required ? 'form-group--error' : ''">
            <b-form-input type="text" v-model.trim="$v.form.commission.$model" />
          </b-form-group>
          <b-form-group :label="$t('Package.additionalCommission')">
            <b-form-input type="text" v-model="form.addcommission" />
          </b-form-group>
          <b-form-group :label="$t('Package.additionalPrice')" :class="$v.form.price.$error && !$v.form.price.required ? 'form-group--error' : ''">
      ...
      <b-col cols="12" sm="12" md="12" lg="8" xl="8" class="mb-3">
        <b-card :title="$t('Phone.general')" class="shadow">
          <b-row>
            <b-col cols="12" sm="4">
              <b-form-group :label="$t('Package.talk')" :class="$v.form.talk.$error && !$v.form.talk.required ? 'form-group--error' : ''">
                <b-form-input type="text" v-model.trim="$v.form.talk.$model" />
              </b-form-group>
            </b-col>
            <b-col cols="12" sm="4">
              <b-form-group :label="$t('Package.internet')" :class="$v.form.internet.$error && !$v.form.internet.required ? 'form-group--error' : ''">
                <b-form-input type="text" v-model.trim="$v.form.internet.$model" />
              </b-form-group>
            </b-col>
            <b-col cols="12" sm="4">
              <b-form-group :label="$t('Package.sms')" :class="$v.form.sms.$error && !$v.form.sms.required ? 'form-group--error' : ''">
                <b-form-input type="text" v-model.trim="$v.form.sms.$model" />
              </b-form-group>
            </b-col>
          </b-row>
          <b-form-group :label="$t('Package.content')">
...
export default {
  ...
  computed: {
    ...mapState(['editorOption', 'returnCount', 'returnLog', 'allTariffs', 'allBrands'])
  },
  watch: {
    simonly: function (e) {
      if (e == true) {
