...
class Sidebar extends Component {
    ...
    routeActive(paths) {
        ...
        paths = paths.map(p => p.replace(BASE_PATH, ""))
        return paths.some(p => this.props.location.pathname.replace(BASE_PATH, "").indexOf(p) > -1 ) 
    }
                        ...
                        <ul className="sidebar-nav">
                            { /* Iterates over all sidebar items */ }
                            {
                                Menu.map((item, i) => {
                                    ...
                                    else if(item.roles?.includes(this.props.userRole)){
